...
void	chapitre_0(t_perso *joueur)
{
	...
	ft_pass();
	printf("\nLe courageux guerrier a la force sans égale.\n\nLe puissant mage aux connaissances ocultes.\n\nLe mystérieux voleur aux ruses multiples.\n\n");
	ft_pass();

	printf("Veuillez écrire la classe qui vous intéresse : Guerrier, Mage ou Voleur\n");
	while (!joueur->classe)
	{
		tmp = ft_get_line();
	...
	if (joueur->classe == VOLEUR)
		printf("voie des gens de l'ombre !\n");
	ft_pass();
	printf("Maintenant que nous avons déterminé ta classe, il nous faut savoir comment tu\nt'appelles ! Quel est donc ton nom ?\n");
